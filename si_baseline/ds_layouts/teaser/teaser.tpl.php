...
if (isset($_GET['dpm-teaser'])) {
  ...
  if (empty($link_path) &&  $element['#view_mode'] == 'teaser_long') {
    if ($element['#entity_type'] == 'taxonomy_term' || $element['#entity_type'] == 'node'){
      $entity = $element['#entity_type'] == 'taxonomy_term' ? $element['#term'] : $element['#node'];
...
$classes_media = ' class="'. $classes_media .'"';
  ...
  $classes_content = 'b-text-wrapper';
...
<div class="<?php print $classes; ?><?php print $media && $media != "&nbsp;" ? ' has-media' : '' ?>"<?php print $attributes; ?>>
  ...
  <?php if ($teaser_content && $teaser_content != "&nbsp;"): ?>
    <div<?php print $classes_content; ?>><?php print ($teaser_content); ?></div>
  <?php endif; ?>
